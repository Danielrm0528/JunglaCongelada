...
fondoJungla = pygame.transform.scale(fondoJungla, (pantallaLargo/2, pantallaAncho/2))
fondoJungla_rect = fondoJungla.get_rect()
fondoJungla_rect.topleft = (0, 0)
...
def terminado(nivelActual,puntajeActual):
    ...
    pantallaAncho = 600
    pantalla = pygame.display.set_mode((pantallaLargo, pantallaAncho))
    botonDevolver = Boton(pantalla.get_width()//2  - 50,500,125,50,(38, 42, 86))
    corriendo = True
...
def final(texto):
    ...
    pantalla.blit(textoFinal,textoFinalrect)
    botonFinal = Boton(pantalla.get_width()/1.2,pantalla.get_height()/1.1, 125, 50, (38, 42, 86))
    corriendo = True
    while corriendo:
        for evento in pygame.event.get():
            if evento.type == pygame.QUIT:
...
def inicio():
    ...
    while corriendo:
        ...
        pantalla.blit(fondoJungla, fondoJungla_rect)
